...
from lib.utils.rename.rename_file import rename_file
...
from lib.utils.path.get_path import get_screenshot_path, get_reports_path
from lib.utils.path.get_path import get_tests_path, get_features_path, get_lint_path
from lib.utils.sleep.time_sleep import time_sleep
from lib.utils.verify.verify_path_exists import verify_path_exists
from lib.utils.json_dict.get_settings import get_model_settings, get_platform_settings
from lib.utils.json_dict.get_settings import get_headless_settings, get_lint_settings
...
class Interact:
    ...
    def teardown():
        if Settings.platform in ("firefox", "chrome", "edge", "ie", "opera"):
            ...
            try:
                Interact.close_browser()
                Interact.quit_appication()
                Interact.click('name', 'Close')
            except WebDriverException:
                pass
            kill_driver()
        elif Settings.platform == "mobile":
    ...
    # Método estático que realiza o fechamento de uma aplicação Mobile
    @staticmethod
    def close_application():
        instance.driver.close_app()

    # Método estático que realiza o fechamento de uma aplicação Windows Desktop
    @staticmethod
    def quit_appication():
    ...
    # Método estático que renomeia um arquivo
    @staticmethod
    def run_rename_file(path, file, new_file):
        return rename_file(path, file, new_file)
    ...
    # Método estático que faz a chamada das funções de carregamento das configurações do arquivo settings.json
    @staticmethod
    def load_settings():
        get_model_settings()
    ...
    # Método estático que faz chamada da função para captura da data e hora atual
    @staticmethod
    def call_get_date_time_now(date_time):
        return get_date_time_now(date_time)

    # Método estático que faz chamada da função para captura do diretório dos reports das execuções dos testes
    @staticmethod
    def call_get_reports_path(type):
        return get_reports_path(type)

    # Método estático que faz chamada da função para captura do diretório dos testes do Pytest
    @staticmethod
    def call_get_tests_path():
        return get_tests_path()

    # Método estático que faz chamada da função para captura do diretório das features do BDD
    @staticmethod
    def call_get_features_path():
        return get_features_path()

    # Método estático que faz chamada da função para captura do diretório de verificação do lint
    @staticmethod
    def call_get_lint_path():
        return get_lint_path()
